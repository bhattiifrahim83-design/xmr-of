...
def get_db() -> PostgresDB:
    global _DB
    # If we previously fell back to NullDB, retry the real connection every call
    # so that a transient startup failure doesn't permanently disable persistence.
    if _DB is not None and not getattr(_DB, 'is_null', False):
        return _DB
    try:
        _DB = PostgresDB()
        ...
        logger.error(
            "PostgresDB connection FAILED — data will NOT be persisted! "
            f"Error: {e}. Check PGHOST/PGPASSWORD/PGSSLMODE env vars."
        )
        class NullDB:
            is_null = True
            ...
            def save_bucket_data(self, *a, **k):
                logger.error("NullDB: save_bucket_data called — DB is unavailable, data lost!")
                return False
            def close(self):
                return None
        _DB = NullDB()
